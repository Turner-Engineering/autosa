...
def get_header_frame(single_tab):
    """1st frame is the header"""
    header_frame = ctk.CTkFrame(single_tab, fg_color="yellow")
...
def get_button_frame(single_tab):
    """2nd frame creates and sets up bands B0-B4, B5h-B7h, B5v-B7v"""
    button_frame = ctk.CTkFrame(single_tab)
...
def get_single_band_layout(single_tab):
    """calls the two frames"""
    single_tab.rowconfigure([0, 1], weight=1)
    single_tab.columnconfigure(0, weight=1)

    # FRAME 1: header and run note
    get_header_frame(single_tab)

    # FRAME 2: Button Frame
    get_button_frame(single_tab)
